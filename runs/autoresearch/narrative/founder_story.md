...
I built an autonomous research system that ran 265+ experiments on a single problem, validated results across six unrelated domains, and produced full forensic logs for every run — and I never wrote a line of Python.

That sentence is the company.

For months I ran the whole proof of concept through pure natural-language steering: I told coding agents what to investigate, what protocol to hold, when to pivot, when to self-reflect on a stall — and the agents wrote every line of code. Fraud detection, EUR/USD signals, Higgs classification, medical-image OOD detection, face clustering, LLM training. Six domains, one protocol, zero hand-written Python. On a consumer Intel laptop that blue-screened under load more times than I counted — which is why the system checkpoints aggressively enough that a BSOD costs minutes, not campaigns. The laptop was not a constraint I overcame; it was a design input. If the loop survives my hardware, it runs on anyone's.
