...
2. **F2 — Serial, daytime-only iteration.** A human runs roughly 1–3 meaningful experiments per day, 8 hours a day (landscape Tier 7 [A37]–[A40]); that day costs $1,116 raw at the $279k average total comp [C25], $1,400–1,700 loaded at 1.25–1.5x employer loading — conservative arithmetic here uses the $1,100 raw low end. The experiments themselves take seconds to minutes at PoC scale (36 s–2.1 min per training run, paper Table C.1, founder-reported) — the bottleneck is the human between them.
...
**Multiplier: 1.2x (band 1.2–1.6x)**, derived from the 20–40% F1 loss: full recovery would give 1/(1−0.2)=1.25x to 1/(1−0.4)=1.67x; assuming only about half that time is actually recoverable gives 1.11–1.25x; we book 1.2x conservative, 1.6x upper.
**Double-count control (explicit):** the human baseline of 1–3 experiments/day in M2 is *gross* of F1 — harness overhead is part of why the number is that low. M1 and M2 therefore overlap, and the overlap is absorbed by the ÷3 global honesty discount applied to the final product (see "The product") rather than by silently multiplying both at full strength and calling it conservative.

### M2 — 24/7 fixed-duration agent experimentation (removes F2, absorbs the 24/7 term)

The agent runs the 7-step loop continuously; each experiment is a bounded, fixed-duration run (minutes at PoC scale), and checkpointing every experiment plus every 5 minutes of reasoning makes multi-day campaigns survivable on consumer hardware (repo `core/checkpoint.py`; paper §3.4). Capability basis: frontier-agent 50% time horizons reached ~12 h by mid-2026, doubling every ~4 months [B1][B2] — Ascent needs reliable ~1 h chunks, not 30 h heroics, which the frontier clears with margin.
**Canonical throughput number (used pack-wide):** the PoC *sustained* ~3–4 experiments/day over months (265+ total on EUR/USD, founder-reported) — and those are gate-passed, documented experiments, versus the human's 1–3 ungated ones. Short-trial tasks *project* to 10–40 gate-passed experiments per night from Table C.1 per-experiment timings (22 s–2.1 min training runs) — a projection, not yet demonstrated end-to-end.
**Quality adjustment on the projected rate:** a human's experiments embed more judgment per step than an agent's minimal changes, so we discount projected agent experiments to 0.2–0.5 human-experiment equivalents (assumption: judgment discount; consistent with RE-bench showing humans 2x agents at 32 h budgets while agents win 4x at 2 h [B4]), giving 2–20x from the projected rate.
**Multiplier: 3x (band 3–20x)** — the conservative 3x is anchored on the *demonstrated* sustained rate (3–4 gate-passed/day vs 1–3 ungated human experiments/day, taking the middle of both ranges); the 20x upper end comes from the projected night-rate after the judgment discount.
...
**Multiplier: 1.1x (band 1.1–1.25x)** on final achieved metric quality — kept deliberately small because it is the least separable term. **Unit conversion, so the product's units stay consistent:** reaching a given metric level requires roughly 10–20% fewer kept improvements when search is grounded (reading the ablation's 18%-lower plateau inversely), so we book the 1.1x as equivalent-kept-improvement productivity — the same output unit as M2 — not as extra metric points multiplied on top.

### M4 — Deterministic audit gates + reproducible evidence bundles (removes F4)

Three pre-commit gates run programmatically: data integrity (split overlap), citation rigor (bibliographic completeness against retrieval), reasoning completeness (word-count floors, required quantitative prediction) — the runner refuses to launch experiments that fail them by default (repo `core/runner.py`; the one documented bypass flag, `--bypass-reasoning-gate`, sentinel-tags the entry `TODO-REWRITE` and surfaces it red on the dashboard — a disclosed, visible exception, not a silent hole). Kept champions ship as self-contained winner archives: config, frozen code, model, 14-section audit report, reproduction log, Colab notebook (repo `core/winner_archive.py`). The gate classes map one-to-one onto the independently documented failure modes of unaudited systems [D20][D21][A46]; deterministic checks (citation exists, split disjoint, hash matches) are the gate class the literature supports, and LLM-judged checks are treated as screens only [D33][D34] (positioning §4.5). What the buyer saves: re-verification drops from re-deriving the result to re-running a one-command bundle.
**Multiplier: 1.3x (band 1.3–2x)** human time per trusted result. Honest caveat carried from survey §7: the gates' catch rates against seeded violations are **unmeasured** — the multiplier's basis is the deterministic-gate literature plus the founder-reported ablation (citation gate removed → invalid-experiment rate spiked 42%, 3 leakage incidents appeared; paper C.2), not a calibrated measurement. Measuring catch rates is the calibration-corpus program (A11, deep_dives §3). *Footnote on a numeric coincidence: the ablation's 42% invalid-experiment spike (founder-reported, paper C.2) and Beel et al.'s 42% experiment-failure rate for the AI Scientist [D20] are independent measurements from unrelated sources that happen to share the value — neither was derived from the other.*

### M5 — Cost-axis substitution (prices the day, does not multiply output)

A 24/7 steering loop routed BYOK-first (A12) costs $3–12/day on Haiku-4.5/Gemini-Flash-tier for routine steps and $20–60/day if run Sonnet-5-tier, computed from Aug 2026 list prices with caching [B18][B20][B22]; observed heavy-automation agent spend lands in the same $17–65/day band [B23]. The engineer's day: $1,116 raw ($279k ÷ 250 [C25]), $1,400–1,700 loaded at 1.25–1.5x. With 30–60 min/day of human steering retained ($69–206 of that day depending on rate), total daily cost is $72–266.
**Cost multiplier: 6x (band 6–15x)** cheaper per research-day — conservative end computed deliberately against the *raw* $1,100 low end and the *high* end of agent cost ($1,100 ÷ ~$185 ≈ 6x); the 15x upper end uses loaded $1,700 against mid-tier routing (~$115/day). Condition (capability_table §3): holds only if routine steps route to mid-tier models — frontier list prices are flat-to-rising [B18][B19][B21].

### The product

Output axis: 1.2 × 3 × 1.1 × 1.3 ≈ **5.1x** more audited kept improvements per human-day (conservative ends only). Cost axis: **6x** cheaper per day (conservative end). Cost per audited kept improvement — the unit a buyer pays for — falls by the product of an output term and a cost term: 5.1 × 6 ≈ 30.9x raw, then **÷3 global honesty discount** — which is where the M1×M2 overlap (the 1–3/day human baseline is gross of harness overhead), residual correlations between terms, and general estimation optimism are absorbed — giving **≈10.4x**. The claim is stated as **≥10x** on that corrected arithmetic, not on the raw product. A skeptic multiplying the upper bands back together gets 1.6×20×1.25×2×15 = 1,200x before any discount — which is exactly why the claim is 10x and not the top of the band.

## §3 Full-spectrum applicability

One system, no "lite" fork — the constitution adapts rigor vocabulary per domain (BRIEF).

1. **Edge-low — the domain scientist.** Priya, a computational-biology postdoc (personas.md) with a dataset and an AUROC target, cannot build a leakage-free harness; today she either doesn't do the ML or does it badly (the [D5] population). For her the gains are *larger* than §2's estimate: F1 is not 30% of her time but a hard blocker. Ascent's wizard builds the protocol; the gates supply the discipline she has no training to apply. Her artifact: a reproduction bundle a reviewer can rerun in one command.
2. **Beachhead — the solo quant / applied-ML engineer.** Marcus, a solo quant (personas.md), knows exactly what a purged fold is — he just can't afford to hand-build one per idea, and he runs 2 experiments a day between meetings. He reads the trial ledger and the per-regime fold table directly; his willingness to act on results is gated on the evidence bundle (riskiest-assumption test, BRIEF). For him the binding gains are M2 + M5.
