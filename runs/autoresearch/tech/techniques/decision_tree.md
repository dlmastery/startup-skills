...
**What a reviewer should notice:** (1) safety and metric-integrity branches structurally outrank progress branches — the tree cannot trade rigor for gains; (2) every branch terminates in the same gate + fixed-duration + verdict spine, so there is no *silent* path around the audit machinery — the one documented bypass flag (`--bypass-reasoning-gate`) sentinel-tags the entry `TODO-REWRITE` and surfaces it red on the dashboard; (3) the NEAR-MISS verdict is the anti-seed-luck valve: borderline wins buy more evidence, not promotion; (4) the tree is the constitution made executable — today the agent runs it as prose judgment, and productization means logging which branch fired per experiment (one more ledger column, feeding the A11 corpus).
