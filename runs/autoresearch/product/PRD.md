...
**What exists today:** a working PoC validated across six domains (financial forecasting, index prediction, tabular classification, medical-imaging OOD, clustering, fraud detection), built solo through natural-language steering, with full forensic logs. Champion test Sharpe +6.52 on EUR/USD after 265+ experiments — a figure that, per our own methodology, is an upper bound on the honest number until deflated for the trial count [D6][D7]; the product ships the deflation machinery as a headline feature, not a footnote (positioning.md §4).

## 2. Goals

1. **G1 — Convert skeptics with evidence, not claims.** A practitioner who distrusts AI output (46% of developers do [C3]) can inspect the trial ledger, rerun any champion with one command, and see exactly which gates fired. Target: of 15 beachhead practitioners shown an audited bundle, ≥8 reproduce it and ≥5 state what evidence would make them act on it (BRIEF riskiest-assumption test).
...
5. **G5 — Instrument the moat from day 1 (A11).** Every discard, every gate verdict, every seeded-violation catch-rate flows into the private calibration corpus; bundles ship champions and run logs, never the negative-result ledger.
...
4. Composite metric designer — expression DSL or callable, SHA-256-fingerprinted at campaign start; runner refuses mid-run metric rewrites (PoC non-negotiable rule 9) [P4][P2].
...
12. Multi-seed / CPCV confirmation runs — champion candidates confirmed distributionally before promotion [P6][P2].
13. Goodhart divergence monitor — tracks composite-proxy vs. each raw target metric; alerts on divergence [survey §6] [P4].
...
15. Trial ledger — append-only record of **every** experiment including discards, with per-trial gate verdicts; the input DSR needs [D6] and the raw material of the calibration corpus (A11) [P2].
...
4. **Immutable protocol under autonomy:** the runner refuses to let the agent rewrite the composite metric, split protocol, or data-integrity invariants mid-run (PoC rule 9) — the agent optimizes inside the sandbox and cannot edit the sandbox.
...
2. **Independent reproductions:** count of bundles rerun by someone other than their author with matching fingerprints. North-star trust metric; target 15-practitioner test ≥8 reproductions (G1).
...
1. **Build the trial ledger + DSR reporting before any new capability** (features 15–16): it is the beachhead persona's stated demand (Marcus card), the positioning's offensive move (§4.6), and the prerequisite of the A11 corpus. Without it, +6.52 stays an unlabeled upper bound.
