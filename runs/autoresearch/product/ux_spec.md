...
- **Local-first:** all screens are self-contained local HTML (PoC dashboard lineage); hosted sync is read-only mirroring, flagged with a "what leaves this machine" link (A9).
- **Theme:** light/dark; dense tables; monospace for hashes/configs.

---

## 1. Instantiation Wizard
...
## 5. Experiment Detail / Reasoning Blob
...
## 7. Winner Archive / Champion Bundle
...
## The micro-interactions that carry the product's feel (cross-screen top 5)

1. **The rising threshold animation** (screen 6) — rigor getting stricter as evidence accumulates is the brand in one motion.
2. **Gate chips that always resolve to text** (global) — nothing green is unexplainable; nothing red is hidden.
3. **The honesty pair** (global) — raw · deflated · N, everywhere, without exception.
4. **"What leaves this machine"** (screens 7, 10, 12) — the A9/A11 data-flow truth one tap away from every export and sync.
5. **Rejection cards that teach** (screens 5, 6) — every discard explains itself; the user gets a methods education as exhaust.

## Recommended next

Spec-to-build order: screens 4 → 6 → 5 → 8 (dashboard, ledger/DSR, experiment detail, steering) — they cover the daily loop and the trust spine and all read from records the PoC already writes. Screen 1 (wizard) next as the activation funnel. Screens 11–12 wait for Team demand and hosted-sync infrastructure respectively. Hand this file plus features_flagship.md to startup-visuals for the collage pass.
