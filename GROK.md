...
This pack was extracted from a 203-exchange, 295-infographic deep-tech startup build (LessonOrca / Aether ΓÇö AI ProbeΓÇôPlanΓÇôTeach tutor). Every skill is **domain-swapped**. The source run is the fidelity floor, not the content.
...
2. **Prompt-without-image.** Writing an image prompt and not calling the image tool is a failure. The source session burned ~40 exchanges on "I don't see the images ΓÇö regenerate." If you emit a prompt, you generate the image in the same turn, then inspect it.
...
|---|---|
| Probe ΓåÆ Plan ΓåÆ Teach | `core_loop` |
| SELPA ΓåÆ genius | `spectrum.low_end` ΓåÆ `spectrum.high_end` |
...
| 6 | `startup-deeptech` | deep-tech use cases + algorithm writeup | 4ΓÇô8 product-screen images |
| 7 | `startup-features` | 20-feature then 50-priority feature lists | 6 + 7 product/collage images |
| 8 | `startup-techniques` | three waves of 50 techniques + decision tree + techniqueΓåöfeature matrix | 5-per-collage until coverage |
| 9 | `startup-market` | TAM/SAM/SOM, landscape, neo-alternatives, comps | 8ΓÇô15 |
...
| 14 | `startup-external-ingest` | novel-only prompts from `external_sources` | 2ΓÇô5 per source |
...
Text phases may start in parallel after params exist (1ΓÇô5). Image phases are sequential enough to keep visual language consistent: lock the visual spec in phase 6, then reuse it.
...
- Generate 1ΓÇô4 images per turn. After each batch, inspect the file. If the image is a prompt card, a blank, or garbled, regenerate once; if still wrong, rebuild the dense board in HTML/CSS per the imagine skill and screenshot.
...
## Anti-patterns (observed in the source session ΓÇö do not repeat)
...
| "Comps are obvious, skip neo-alternatives" | Source required named alternative programs, then forced regeneration of the missing 5ΓÇô10. |
