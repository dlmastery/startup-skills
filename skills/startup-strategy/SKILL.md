...
8. **`business_model_canvas.md`** — the Osterwalder nine blocks (customer segments, value propositions, channels, customer relationships, revenue streams, key resources, key activities, key partners, cost structure), each with **one falsifiable hypothesis and the cheapest test that would kill it**. This is Blank's Customer Discovery Phase 1 artifact; the Lean Canvas is Maurya's variant and drops Key Partners, Key Activities and Customer Relationships — keep both, and cross-link them.
9. **`petal_diagram.md`** — Blank's petal, **not a 2×2**: place the company at the centre and name the 4–6 **adjacent markets** it will draw customers *from*, with the incumbents in each petal and what those customers currently spend. The question is which existing budgets and habits this displaces, which is harder and more useful than where rivals sit on two axes.
10. **`channel_plan.md`** — channel map by segment **plus channel economics**: for each channel, the discount/margin stack from list price to net, the cost to acquire through it, the time to first revenue, and whether it is viable at this price point. A GTM narrative that names channels without computing their economics is asserting distribution, not planning it.
11. **`sales_roadmap.md`** — Blank's Customer Validation Phase 1 set: the **organisation map** (who sits where in the buyer), the **influence map** (who actually sways the decision), the **access map** (how a stranger reaches them), and the sales process with the artifacts needed at each step.
