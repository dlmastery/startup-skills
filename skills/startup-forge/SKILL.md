...
**Read `references/quality-bar.md` first. It binds every phase.** Then read `references/writing-style.md`: every sentence of prose in the run is ASD-STE100, and `templates/check_ste.js` checks it at every gate.
...
5. **Domain vocabulary everywhere.** Use the nouns from `BRIEF.md` §Vocabulary, never placeholder nouns like "the product" or leftover nouns from other domains. §Vocabulary is also the STE technical-name list: one name per thing, and the linter exempts it.
