...
2. **Spot-check quality.** Sample 3–5 present artifacts against `references/quality-bar.md`: unsourced numbers? adjective-claims? missing "recommended next" endings? Mermaid that doesn't parse? HTML that doesn't render? Log failures as `fix` rows.
   - **Check property 9 (ASD-STE100) on every artifact, not a sample** — it is mechanical: `node templates/check_ste.js runs/<slug> --vocab runs/<slug>/BRIEF.md`. Any file with a `DASH`, `SEMI`, `LONG`, or `PARA` hit is a `fix` row that names the line. Put the by-rule totals in `COVERAGE.md` so the next session can see the trend.
